...
switch ($Action) {
    "Test" { $projectdir = Join-Path -Path $testDir -ChildPath 'Falco.OpenApi.Tests' }
    "Pack" { $projectDir = Join-Path -Path $srcDir -ChildPath 'Falco.OpenApi' }
    Default { $projectDir = Join-Path -Path $srcDir -ChildPath 'Falco.OpenApi' }
}
